...
export const heroTemplates = mysqlTable("hero_template", {
  ...
  templateClass: varchar("template_class", { length: 24 }),
  acquisitionWeight: int("acquisition_weight", { unsigned: true }),
  appearanceCode: varchar("appearance_code", { length: 96 }),
  namePool: json("name_pool").$type<string[]>(),
  growthFocus: varchar("growth_focus", { length: 120 }),
  growthModifiers: json("growth_modifiers").$type<Record<"strength"|"magic"|"technique"|"speed"|"constitution"|"armor"|"resistance",number>>(),
