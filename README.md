...
- 已导入《昆吾无名录》P0–C13 完整主线，共 15 章、151 个场景/阶段；当前事实源为 `1.0-story-baseline.6 + story-baseline.3`。
- 已导入 7 条世界支线、4 条个人支线和 2 条结局专属任务，共 13 条任务链、70 个场景/阶段。
- 每个导入场景保留草案原文快照与来源文件，可审计台词、选择和检定是否遗漏。

## 从 KunWu 草案同步剧情

后台通过只读方式解析相邻 `KunWuGodot/Docs/1.0策划案` 中的活动事实源（可用 `KUNWU_GODOT_ROOT` 覆盖）：

- `剧情/20_1.0剧情任务详细剧本_草案.md`：P0–C04 主线与五条 1.0 详细支线。
- `24_完整游戏剧情总纲_草案.md`：宏观规则、事实源边界与结局条件。
- `25_完整游戏主线详细剧情_草案.md`：C04 衔接、C05–C13、终战与三结局。
- `26_完整游戏支线剧情库_草案.md`：完整游戏的十三条支线任务链。
...
导入器会生成完整主线与完整支线两份版本化 JSON；审计器独立核对 221 个场景/阶段、931 句台词、234 个选择/检定或终局结果、原文快照、入口和连线。不会修改 KunWuGodot 项目的策划文档。
...
复用现有 Docker MySQL，新增资源域表；玩家生产、余额与进度由服务端结算。

```bash
pnpm resources:setup
pnpm resources:dev
# 或先 pnpm build，再 pnpm resources:start
```

打开 [资源管理](http://127.0.0.1:3100/resources)，连接本机开发身份后，可管理资源目录、配置审核发布、生产模拟、玩家余额与流水。Godot 工程运行 `run_resource_online_test.command` 接入同一服务，使用独立测试档，不覆盖原存档。

验证：`pnpm test:resources`（含真实 MySQL），服务启动后执行 `pnpm test:resources:godot`。本机凭据保存在被 Git 忽略、权限 600 的 `.local/`，不可提交。

完整操作、接口和边界见 [资源模块说明](server/domain/resources/README.md)。正式登录、旧档迁移与其他玩法全面接入统一账本仍是后续上线集成项。
